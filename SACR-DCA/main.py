...
def train2(x,adj,n_clusters):
    ...
    all_loss = loss_coef*0.1+ +loss_instance*0.1+rec_loss*0.001+cs_loss* 0.01
    all_loss.backward()
    optimizer.step()
...
num_class = 32
############## load data/graph #####################
true_protein,protein_sample_name=load_protein(data_fold,sub_name)
true_protein = true_protein.cuda()

for type in dataType:
    omic_dic[type] = load_data(type, sub_name, data_fold)
...
final_pro =final_pro.cpu().detach().numpy()
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
if not isinstance(final_pro, torch.Tensor):
    final_pro = torch.from_numpy(final_pro).float()
