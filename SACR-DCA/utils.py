...
def load_protein(data_fold,sub_name):
    fea_protein_file = data_fold + '/protein/' + sub_name + '/protein.fea'
    df_protein = pd.read_csv(fea_protein_file, sep=',', header=0, index_col=0)
    df_protein = df_protein.T
    protein_sample_name = df_protein.index
    true_protein = torch.FloatTensor(df_protein.values.astype(float))

    return true_protein,protein_sample_name

def gen_trte_adj_mat(data_tr,num_class=6):
    adj_metric = "cosine"
    adj_list = []
    adj_list.append(gen_adj_mat_tensor(data_tr, num_class, adj_metric))
    return adj_list
def load_data(data_type,sub_name,data_fold):
    file_input = data_fold
    fea_save_file =file_input +'/omics_data/'+sub_name+'/' +data_type + '.fea'
    if isfile(fea_save_file):
        df_fea = pd.read_csv(fea_save_file, sep=',', header=0, index_col=0)
        df_fea=df_fea.T
    return df_fea
